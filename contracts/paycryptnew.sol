...
/**
 * @title Paycrypt
 * @dev ERC20-based decentralized payment system with multi-chain router support
 * @author Paycrypt Team
 */
contract Paycrypt is Ownable, ReentrancyGuard, Pausable {
    ...
    enum OrderSource { Direct, Router }
    ...
    modifier onlyAuthorizedRouter() {
        require(_authorizedRouters[msg.sender], "Router not authorized");
        _;
    }
    ...
    /**
     * @dev Create a new order (direct from user)
     */
    ...
    {
        _createOrder(requestId, msg.sender, tokenAddress, amount, OrderSource.Direct, true);
    }

    /**
     * @dev Create order from authorized router (cross-chain)
     */
    function createOrderFromRouter(
        bytes32 requestId,
        address user,
        address tokenAddress,
        uint256 amount
        ...
        tokenSupported(tokenAddress)
    {
        _createOrder(requestId, user, tokenAddress, amount, OrderSource.Router, false);
    }
    ...
    function _createOrder(
        ...
        _userData[user].orders.push(_orderCounter);

        emit OrderCreated(_orderCounter, requestId, user, tokenAddress, amount, source);
    }

    /**
     * @dev Create order with service type for router (enhanced version)
     */
    function createOrderFromRouterWithService(
        bytes32 requestId,
        address user,
        address tokenAddress,
        uint256 amount,
        ...
        tokenSupported(tokenAddress)
    {
        ...
        // Emit payment settled event for router integration
        emit PaymentSettled(requestId, user, amount, serviceType, "Pending");
    }
        ...
        orderPending(orderId) 
    {
        ...
        emit PaymentSettled(order.requestId, order.user, order.amount, "Service", "Successful");
    }
        ...
        orderPending(orderId) 
    {
        ...
        emit PaymentSettled(order.requestId, order.user, order.amount, "Service", "Failed");
    }
    ...
    function batchProcessOrders(
        ...
        OrderStatus[] calldata statuses
    ) external nonReentrant onlyAdmin {
        require(orderIds.length == statuses.length, "Array length mismatch");
        require(orderIds.length <= 50, "Batch size too large");

        for (uint256 i = 0; i < orderIds.length; i++) {
            uint256 orderId = orderIds[i];
            OrderStatus status = statuses[i];

            if (orderId > 0 && orderId <= _orderCounter && 
                _orders[orderId].status == OrderStatus.Pending) {
                
                if (status == OrderStatus.Successful) {
                    _processSuccessfulOrder(orderId);
                } else if (status == OrderStatus.Failed) {
                    _processFailedOrder(orderId);
                }
            }
        }
    }
    ...
    /**
     * @dev Internal function to process successful order
     */
    function _processSuccessfulOrder(uint256 orderId) private {
        Order storage order = _orders[orderId];
        SupportedToken storage token = _supportedTokens[order.tokenAddress];
    ...
    /**
     * @dev Internal function to process failed order
     */
    function _processFailedOrder(uint256 orderId) private {
        Order storage order = _orders[orderId];
        SupportedToken storage token = _supportedTokens[order.tokenAddress];
    ...
    /**
     * @dev Authorize router contract
     */
    function authorizeRouter(address router) 
        external 
        onlyOwner 
        validAddress(router) 
    {
        if (!_authorizedRouters[router]) {
            _authorizedRouters[router] = true;
            _routerList.push(router);
            emit RouterAuthorized(router);
        }
    }

    /**
     * @dev Deauthorize router contract
     */
    function deauthorizeRouter(address router) 
        external 
        onlyOwner 
        validAddress(router) 
    {
        if (_authorizedRouters[router]) {
            _authorizedRouters[router] = false;
            
            // Remove from router list
            for (uint256 i = 0; i < _routerList.length; i++) {
                if (_routerList[i] == router) {
                    _routerList[i] = _routerList[_routerList.length - 1];
                    _routerList.pop();
                    break;
                }
            }
    ...
    function getAuthorizedRouters() external view returns (address[] memory) {
        return _routerList;
    }
    ...
    function getOrdersByStatus(OrderStatus status) external view returns (uint256[] memory) {
        ...
        // First pass: count orders with the specified status
        for (uint256 i = 1; i <= _orderCounter; i++) {
            if (_orders[i].status == status) {
                count++;
            }
        }
    ...
    function getOrdersByUser(address user, OrderStatus status) external view returns (uint256[] memory) {
        uint256[] memory userOrders = _userData[user].orders;
        uint256 count = 0;
        
        // First pass: count orders with the specified status
        for (uint256 i = 0; i < userOrders.length; i++) {
            if (_orders[userOrders[i]].status == status) {
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < userOrders.length; i++) {
            if (_orders[userOrders[i]].status == status) {
                filteredOrders[index] = userOrders[i];
                index++;
            }
        }
